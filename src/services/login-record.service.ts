...
import { ILoginRecordInput } from "@types";
...
const getLoginRecordsByUserId = async (userId: string) => {
  try {
    ...
    console.error(`[getLoginRecordsByUserId] Error: ${error.message}`, {
      userId,
      stack: error.stack,
    });
...
const createLoginRecord = async (loginRecord: ILoginRecordInput) => {
  try {
    return await LoginRecord.create(loginRecord);
  } catch (error: any) {
    console.error(`[createLoginRecord] Error: ${error.message}`, {
      loginRecord,
      stack: error.stack,
    });
...
export { getLoginRecordsByUserId, createLoginRecord };
