import { ConflictError, ForbiddenError, InternalServerError } from "@errors";
  ...
  async checkAndLockAccountOnFailure(
    ...
    if (!savedFailure) {
      ...
      throw new InternalServerError(
        ...
        "SAVE_FAILED",
        {
          loginFailure: "LOGIN_FAILURE_UNSAVED",
        }
      );
    }
  ...
  async clearNormalLoginFailures(userId: string): Promise<void> {
    ...
    if (response?.deletedCount !== normalIds.length) {
      ...
      throw new ConflictError(
        ...
        "DELETE_FALIED",
        {
          loginFailure: "PARTIAL_DELETION_FAILED",
        }
      );
    }
  }
