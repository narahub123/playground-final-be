...
const checkEmailDuplication = async (email: string): Promise<boolean> => {
  try {
    ...
    console.error(`[checkEmailDuplication] Error: ${error.message}`, {
      email,
      stack: error.stack,
    });
 ...
 */
const checkUserIdDuplication = async (userId: string) => {
  try {
    ...
    // 오류 발생 시 콘솔에 에러 정보 출력
    console.error(`[checkUserIdDuplication] Error: ${error.message}`, {
      userId,
      stack: error.stack,
    });
...
// 계정 잠금 업데이트
const updateLockStatus = async (userId: string, lockStatus: ILockStatus) => {
  try {
    return await User.updateOne({ userId }, { $set: { lockStatus } });
  } catch (error: any) {
    console.error(`[updateLockStatus] Error: ${error.message}`, {
      userId,
      stack: error.stack,
    });
